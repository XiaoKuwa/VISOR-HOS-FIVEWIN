...
if __name__ == '__main__':
    ...
    parser.add_argument('--epick_visor_store', type=str, required=True,
                        default='/path/to/epick_visor/GroundTruth-SparseAnnotations',
                        help='Folder saving EPIC-KITCHENS VISOR data.')
                        ...
                        for d_idx, entity in enumerate(entities):
                            ...
                            if entity['name'] in glove_ls and 'on_which_hand' in entity:
                                if entity['on_which_hand'] is not None and len(entity['on_which_hand']) == 2:
                                    print(
                                        f">> wrong 'on_which_hand' in {item['image']['name']}, {entity['on_which_hand']} to ['left hand', 'right hand']")
                                    info['video_annotations'][i_idx]['annotations'][d_idx]['on_which_hand'] = [
                                        'left hand', 'right hand']
                            # correct contacted obj
                            if name in correct_dict:
                                if entity['name'] in correct_dict[name]:
                                    print(name, entity['name'], 'before:',
                                          info['video_annotations'][i_idx]['annotations'][d_idx]['in_contact_object'],
                                          'after:', correct_dict[name][entity['name']])
                                    info['video_annotations'][i_idx]['annotations'][d_idx]['in_contact_object'] = \
                                    correct_dict[name][entity['name']]
                ...
                for item in v_annotations:
                    ...
                    for e_idx, entity in enumerate(entities):
                        ...
                        if entity['name'] in ['left hand', 'right hand']:
                            ...
                            h_bbox_xyxy = [h_bbox_xywh[0], h_bbox_xywh[1], h_bbox_xywh[0] + h_bbox_xywh[2],
                                           h_bbox_xywh[1] + h_bbox_xywh[3]]
                            ...
                            if in_contact_object_id in ['none-of-the-above', 'inconclusive']:
                                print(f'>> Error: hand should not have invalid label')
                                pdb.set_trace()
                                continue
                                ...
                                if args.combine_on_hand_glove_w_hand and in_contact_object['name'] in glove_ls:
                                    # for on-hand gloves
                                    if in_contact_object['on_which_hand'] is not None and entity['name'] in \
                                            in_contact_object['on_which_hand']:
                                        # combine new hand mask = hand masks + on-hand glove
                                        h_masks, h_masks_coco = get_masks(
                                            entity['segments'] + in_contact_object['segments'])
                                        h_bbox_xywh = get_bbox(h_masks)
                                        h_bbox_xyxy = [h_bbox_xywh[0], h_bbox_xywh[1], h_bbox_xywh[0] + h_bbox_xywh[2],
                                                       h_bbox_xywh[1] + h_bbox_xywh[3]]
                                        h_area = get_area(h_masks, height=h, width=w)
                                        ...
                                        if glove_in_contact_object_id in ['none-of-the-above', 'inconclusive', None]:
                                            ...
                                            isincontact = 1
                                            glove_in_contact_object = \
                                            [x for x in entities if x['id'] == glove_in_contact_object_id][0]
                                            print(img_path, entity['name'], in_contact_object['name'],
                                                  glove_in_contact_object['name'])
                        ...
                        # for on-hand gloves
                        elif args.combine_on_hand_glove_w_hand and entity[
                            'name'] in glove_ls and 'on_which_hand' in entity and entity['on_which_hand'] not in [
                            'inconclusive', None]:
                            ...
                            if 'on_which_hand' not in entity:
                                print(
                                    f">> {item['image']['image_path']}, {entity['name']} does not have 'on_which_hand' label")
                                pdb.set_trace()
                                continue
